        ...
        <div className="max-w-2xl mx-auto">
          ...
          <div className="mb-8 relative">
            <div className="relative">
              <Search className="absolute right-3 top-1/2 -translate-y-1/2 text-text-secondary dark:text-text-muted w-5 h-5" />
              <Input
                type="text"
              ...
              />
              {isSearching && (
                <Loader2 className="absolute left-3 top-1/2 -translate-y-1/2 animate-spin text-primary w-5 h-5" />
              )}
            </div>

            {/* Results - Text Only ⚠️ */}
            {searchResults.length > 0 && (
              <div className="absolute top-full left-0 right-0 mt-2 bg-white dark:bg-surface-elevated rounded-xl shadow-elevation-2 z-50 max-h-64 overflow-y-auto border border-primary/10 dark:border-border-subtle">
                {searchResults.map((perfume) => (
                  <button
                    key={perfume.id}
                    onClick={() => handleAddPerfume(perfume)}
                    className="w-full text-right p-4 hover:bg-cream-bg dark:hover:bg-surface-muted transition-colors border-b dark:border-border-subtle last:border-b-0 flex flex-col"
                  >
          ...
          <div className="mt-12 flex flex-col gap-4">
            ...
            <button
              onClick={handleSkip}
              disabled={isPending}
              className="text-text-secondary dark:text-text-muted hover:text-primary dark:hover:text-amber-500 transition-colors text-sm underline disabled:opacity-50 disabled:cursor-not-allowed"
            >
              تخطي هذه الخطوة
            </button>
          </div>
  ...
  return (
    <div className="flex items-center gap-4 p-3 bg-white dark:bg-surface rounded-xl shadow-elevation-1 border border-primary/5 dark:border-border-subtle hover:shadow-elevation-2 transition-all">
      ...
      <button
        ...
        className="p-3 hover:bg-danger-red/10 dark:hover:bg-red-500/20 rounded-xl transition-colors min-touch-target"
