    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-cream-bg dark:!bg-zinc-900" dir={direction}>
        <div className="text-center space-y-6 px-6">
          ...
          <p className="text-[#0EA5A4]">{t('auth.pleaseSignIn')}</p>
          <Link href="/login">
    ...
    <div className="min-h-screen bg-cream-bg dark:!bg-zinc-900 pb-20" dir={direction}>
      <div className="bg-white border-b border-primary/5 pt-12 pb-8 px-6 text-center">
        <div className="max-w-2xl mx-auto">
          ...
          <p className="text-[#0EA5A4] text-sm">{session?.user?.email}</p>
          {tierData && (
            <div className="mt-3 flex flex-wrap justify-center gap-2">
              {tierData.tier === 'PREMIUM' && tierData.hasActiveSubscription ? (
                ...
                <span className="inline-flex items-center gap-1.5 bg-cream-bg text-[#0EA5A4] text-xs font-bold px-3 py-1.5 rounded-full">
      ...
      <main className="max-w-2xl mx-auto px-6 mt-10 space-y-8">
          ...
          <section className="bg-white rounded-[2rem] p-8 shadow-elevation-1 border border-primary/5">
            ...
            {tierData.tier === 'PREMIUM' && tierData.subscription ? (
              <div className="space-y-3">
                ...
                <p className="text-[#0EA5A4] text-sm">
              ...
              <div className="space-y-3">
                <p className="text-[#0EA5A4] text-sm">خطتك الحالية: مجاني</p>
                <Button
                  variant="outline"
        ...
        <section className="bg-white rounded-[2rem] p-8 shadow-elevation-1 border border-primary/5">
          ...
          <div className="space-y-4">
            <div>
              <label className="text-xs font-bold text-[#0EA5A4] mb-1.5 block mr-1">{t('personalInfo.fullName')}</label>
              <Input defaultValue={session?.user?.name || ''} className="rounded-xl py-6" />
            </div>
            <div>
              <label className="text-xs font-bold text-[#0EA5A4] mb-1.5 block mr-1">{t('personalInfo.email')}</label>
          ...
          <div className="space-y-6">
            <div className="flex items-center justify-between p-4 bg-cream-bg rounded-2xl">
              <div>
                ...
                <p className="text-[10px] text-[#0EA5A4]">{t('sensitivity.strictDesc')}</p>
              </div>
              <button
            ...
            <div className="flex items-center justify-between p-4 bg-cream-bg rounded-2xl">
              <div>
                ...
                <p className="text-[10px] text-[#0EA5A4]">{t('sensitivity.ingredientAlertsDesc')}</p>
              </div>
              <button
